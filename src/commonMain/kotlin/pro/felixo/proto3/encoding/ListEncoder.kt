...
class ListEncoder(
    ...
    private fun encodeElement(wireValue: WireValue) = if (packed)
        packedBuffer.encodeValue(wireValue)
    else
        output.encodeField(fieldNumber, wireValue)
    ...
    override fun encodeEnum(enumDescriptor: SerialDescriptor, index: Int) = error("ListEncoder does not support encodeEnum")
    override fun encodeFloat(value: Float) = error("ListEncoder does not support encodeFloat")
    override fun encodeInline(descriptor: SerialDescriptor): Encoder = error("ListEncoder does not support encodeInline")
